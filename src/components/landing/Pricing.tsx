...
const Pricing = () => {
  ...
  const buildFeatures = (plan: any): string[] => {
    ...
    if (plan.max_patients === -1) features.push(`${t.unlimited} ${t.patients}`);
    else if (plan.max_patients) features.push(`${plan.max_patients} ${t.patientsPerMonth}`);

    if (plan.sms_credits === -1) features.push(`${t.unlimited} SMS`);
    else if (plan.sms_credits) features.push(`${plan.sms_credits} ${t.smsCredits}`);

    if (plan.max_staff === -1) features.push(`${t.unlimited} ${t.staffPlural}`);
    else if (plan.max_staff != null) {
      const staffCount = (plan.max_staff || 0) + 1;
      features.push(`${staffCount} ${staffCount > 1 ? t.staffPlural : t.staff}`);
    }

    if (plan.max_chambers === -1) features.push(`${t.unlimited} ${t.chambersPlural}`);
    else if (plan.max_chambers && plan.max_chambers > 1) features.push(`${plan.max_chambers} ${t.chambersPlural}`);

    if (plan.max_prescriptions_per_month === -1) features.push(`${t.unlimited} ${t.prescriptions}`);
    else if (plan.max_prescriptions_per_month) features.push(`${plan.max_prescriptions_per_month} ${t.prescriptions}`);
  ...
  // Filter visible plans, then sort
  const visiblePlans = plans
    ? [...plans]
        .filter((p: any) => p.show_on_landing !== false)
        .sort((a, b) => TIER_ORDER.indexOf(a.tier) - TIER_ORDER.indexOf(b.tier))
    : [];

  // Split: first 3 on top row, rest on bottom row
  const topRow = visiblePlans.slice(0, 3);
  const bottomRow = visiblePlans.slice(3);
      ...
      <div className="container px-4 md:px-6">
        ...
        {isLoading ? (
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            {/* Top row - up to 3 plans */}
            {topRow.length > 0 && (
              <div className={cn(
                "grid gap-8",
                topRow.length === 1 ? "grid-cols-1 max-w-md mx-auto" :
                topRow.length === 2 ? "grid-cols-1 md:grid-cols-2 max-w-3xl mx-auto" :
                "grid-cols-1 md:grid-cols-3"
              )}>
                {topRow.map(renderPlanCard)}
              </div>
            )}

            {/* Bottom row - remaining plans */}
            {bottomRow.length > 0 && (
              <div className={cn(
                "grid gap-8",
                bottomRow.length === 1 ? "grid-cols-1 max-w-md mx-auto" :
                bottomRow.length === 2 ? "grid-cols-1 md:grid-cols-2 max-w-3xl mx-auto" :
                "grid-cols-1 md:grid-cols-3"
              )}>
                {bottomRow.map(renderPlanCard)}
              </div>
            )}
          </div>
        )}
