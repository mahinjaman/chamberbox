                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2 p-4 rounded-lg border bg-muted/30">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {isExpired ? 0 : daysRemaining || 0} days
                      </span>
                    </div>
                    {expiresAt ? (
                      <>
                        <Progress 
                          value={isExpired ? 100 : Math.max(0, 100 - (daysRemaining || 0))} 
                          className="h-2"
                          indicatorClassName={isExpired ? 'bg-destructive' : daysRemaining && daysRemaining <= 7 ? 'bg-yellow-500' : 'bg-primary'}
                        />
                        <div className="text-xs text-muted-foreground">
                          {isExpired ? 'Expired on' : 'Expires'}: {format(expiresAt, 'dd MMM yyyy')}
                        </div>
                      </>
                    ) : (
                      <div className="flex items-center gap-1 text-xs text-green-600">
